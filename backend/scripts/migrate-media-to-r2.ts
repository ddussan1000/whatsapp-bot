...
for (const [k, v] of Object.entries({ SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, R2_ACCOUNT_ID, R2_ACCESS_KEY_ID, R2_SECRET_ACCESS_KEY, R2_BUCKET_NAME, R2_PUBLIC_URL })) {
...
async function migrate() {
  ...
  if (error) { console.error("Error fetching org_media:", error.message); process.exit(1); }
  if (!mediaRows?.length) { console.log("No hay archivos en org_media."); return; }
  ...
  for (const row of mediaRows) {
    ...
    if (updateErr) {
      console.log(`⚠️  Subido a R2 pero error actualizando DB: ${updateErr.message}`);
      continue;
    }
    ...
    console.log(`✅ (${(buffer.length / 1024).toFixed(0)} KB)${count ? ` — ${count} step(s) actualizados` : ""}`);
  }

  console.log("\n✅ Migración completada.\n");
  console.log("Próximos pasos:");
  console.log("  1. Verifica que los archivos estén en R2 (Cloudflare → R2 → flow-media → Objects)");
