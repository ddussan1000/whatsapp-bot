-- Fix: use receipt_date (actual payment date) instead of validated_at for report
-- aggregation. When an agent reviews/validates a receipt on a different day than
-- the receipt itself, sales were incorrectly attributed to the review date.
-- Falls back to validated_at when receipt_date is null (manual payments).
...
begin
  with filtered_payments as (
    select
      ...
      p.validated_at,
      p.receipt_date,
      coalesce(p.receipt_date, p.validated_at) as sale_date,
      p.amount,
      p.currency,
      p.phone,
      p.flow_id,
    ...
    where p.organization_id = p_organization_id
      ...
      and coalesce(p.receipt_date, p.validated_at) >= p_from
      and coalesce(p.receipt_date, p.validated_at) <= p_to
