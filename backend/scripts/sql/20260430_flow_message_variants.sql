-- Add text_variants to flow_step_messages
-- text_variants stores alternative texts for a message slot.
-- When non-empty the engine picks one at random instead of using text_content.
-- text_content remains backward-compatible (used when text_variants is empty).

alter table public.flow_step_messages
  add column if not exists text_variants text[] not null default '{}';

-- Update upsert_flow_tree RPC to handle text_variants
...
begin
  ...
  if v_flow_id is null then
    insert into public.flows (
      ...
      no_match_behavior, system_prompt, is_active, session_timeout_hours,
    ...
    values (
      ...
      coalesce((payload->>'sessionTimeoutHours')::integer, 24),
      v_now,
      v_now
    )
    ...
    set
      ...
      session_timeout_hours = coalesce((payload->>'sessionTimeoutHours')::integer, session_timeout_hours),
      updated_at = v_now
    where id = v_flow_id
    ...
    loop
      ...
      if v_message_id is null then
        insert into public.flow_step_messages (
          ...
          text_content, media_url, filename, caption, text_variants,
          created_at, updated_at
        )
        values (
          v_step_id,
          ...
          coalesce(
            (
              select array_agg(v)
              from jsonb_array_elements_text(coalesce(v_msg->'textVariants', '[]'::jsonb)) t(v)
              where trim(v) <> ''
            ),
            '{}'::text[]
          ),
