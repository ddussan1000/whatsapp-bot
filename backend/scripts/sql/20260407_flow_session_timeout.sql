-- Add per-flow session timeout (hours) to flows table
-- Default is 24h to preserve existing behavior

alter table public.flows
  add column if not exists session_timeout_hours integer not null default 24
    check (session_timeout_hours >= 0);

-- Update upsert_flow_tree RPC to handle session_timeout_hours
