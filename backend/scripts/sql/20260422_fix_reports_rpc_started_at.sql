-- Fix: filter conversations by started_at (when they began) instead of updated_at.
-- Using updated_at was counting old conversations that received a new message on the
-- selected date, inflating the conversations count (e.g. 590 vs the real 147).
