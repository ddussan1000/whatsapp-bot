  ...
  return (
    <Dialog open={Boolean(item)} onOpenChange={(v) => !v && onClose()}>
      <DialogContent showCloseButton className="sm:max-w-2xl">
        ...
        <div className="flex items-center justify-center rounded-lg bg-muted/40 p-4 min-h-[200px]">
...
export function MediaPage() {
  ...
  const stats = {
    ...
    images: allItems.filter((i) => i.media_type === "image").length,
    videos: allItems.filter((i) => i.media_type === "video").length,
    documents: allItems.filter((i) => i.media_type === "document").length,
  };
      ...
      {allItems.length > 0 && (
        <div className="flex flex-wrap gap-3">
          {[
            ...
            { label: "Videos", value: stats.videos, color: "text-violet-600" },
            {
              label: "Documentos",
            ...
            <div
              key={label}
              className="rounded-lg border bg-card px-4 py-2 shadow-sm"
            >
